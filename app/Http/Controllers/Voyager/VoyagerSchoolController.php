...
class VoyagerSchoolController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    public function enrolledStudents($school_id)
    {
        if (Auth::check()) {
            $slug = 'schools';
            $getStudents = DB::table('users_subscription')->where('school_id', $school_id)->get();

            $view = 'voyager::bread.students';

            if (view()->exists("voyager::$slug.students")) {
                $view = "voyager::$slug.students";
            }
            ...
            return redirect(lang_url('/admin'));
        }
    }


    public function overallStudent($school_id, $student_id)
    {
        if (Auth::check()) {
            $slug = 'schools';
            $view = 'voyager::bread.student_performance';

            $student = User::find($student_id);

            $getResults = DB::table('user_access')->where([['user_id', $student->id], ['object_type', 'chapter']])->get();

            if (view()->exists("voyager::$slug.student_performance")) {
                $view = "voyager::$slug.student_performance";
            }
