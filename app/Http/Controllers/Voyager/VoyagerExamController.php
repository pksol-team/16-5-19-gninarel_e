...
class VoyagerExamController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    public function records($exam_id)
    {
        if (Auth::check()) {

            $loggedUser = Auth::user();
            $exam = DB::table('exam')->where('id', $exam_id)->get();

            if ($loggedUser->role_id != '1' && $exam->user_id != $loggedUser->id) {
                return redirect(lang_url('/admin/exam'));
            }
            
            $slug = 'exam';
            $getRecords = DB::table('test_results')->where('exam_id', $exam_id)->get();

            $view = 'voyager::bread.records';

            if (view()->exists("voyager::$slug.records")) {
                $view = "voyager::$slug.records";
            }
